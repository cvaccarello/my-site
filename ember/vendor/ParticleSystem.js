...
var ParticleSystem = function ParticleSystem(cfg) {
	...
	var findEmitterIndex = function(id) {
		...
		if (!matchFound && console && console.error) {
			...
			if (id < eLength) {
				...
				console.error('no emitter could be found at index "'+id+'"');
			}
		}
	...
	this.addEmitter = function(options) {
		...
		var emitter_settings = (options)? $.extend(true, $.extend(true, {}, settings.emitter), options) : settings.emitter;

		// OPTIMIZATION: convert direction in degrees to radians
		var radians = Math.PI/180;
		emitter_settings.direction.min = emitter_settings.direction.min * radians;
		emitter_settings.direction.max = emitter_settings.direction.max * radians;

		// OPTIMIZATION: adjust speed & size to be in milliseconds (instead of pps - pixels per second)
		if (emitter_settings.particle.speed !== 0) emitter_settings.particle.speed /= 1000;
		if (emitter_settings.particle.size !== 0) emitter_settings.particle.size /= 1000;
...
var ParticleEmitter = function ParticleEmitter(settings) {
	...
	var init = function() {
		...
		if (settings.spawn_point == 'center') {
			...
			spawn_position = {x: settings.offset.x + (This.width/2), y: settings.offset.y + (This.height/2)};
		} else {
	...
	this.update = function() {
		...
		if (elapsed_time > 500) {
			if (console && console.log) { console.log('Particle System: Cap elapsed_time at 500 (was '+elapsed_time+')'); }
			elapsed_time = 500;
		}
	...
	this.getParticleSpawnPosition = function() {
		...
		var spawn_point = {x: eOffset.left + spawn_position.x, y: eOffset.top + spawn_position.y};
	...
	this.addParticle = function addParticle(spawn_point, direction) {
		...
		This.particles.push(new settings.particle.constructor(settings.particle, spawn_point, direction));
	};
	...
	this.createParticles = function createParticles(elapsed_time) {
		...
		var emit_delay = (settings.emit_delay.min || settings.emit_delay.max)?
			(Math.random() * (settings.emit_delay.max - settings.emit_delay.min)) + settings.emit_delay.min :
			settings.emit_delay;
	...
	this.toggle = function() {
		...
		if (triggerCallback) { window.clearTimeout(triggerCallback); }
	};
...
var Particle = function Particle(settings, spawn_point, direction) {
	...
	var init = function() {
		...
		This.coordinates = {x: This.coordinates.x - local_coords.left, y: This.coordinates.y - local_coords.top};
